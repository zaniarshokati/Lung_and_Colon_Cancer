...
from tensorflow.keras.layers import Dense, Activation,Dropout,Conv2D, MaxPooling2D,BatchNormalization, Flatten, Add, AveragePooling2D, Input, ZeroPadding2D,Concatenate
...
class HandleData:
            ...
            for cls in dirs:
                if cls not in ["lung_image_sets", "colon_image_sets"]:
                    ...
                    for f in os.listdir(class_path):
                        self.data.append({'filepaths': os.path.join(class_path, f), 'labels': cls})
    ...
    def balance_dataset(self, df, sample_size):
        ...
        sample_list = [group.get_group(label).sample(sample_size, replace=False, random_state=123, axis=0) for label in df['labels'].unique()]
        df = pd.concat(sample_list, axis=0).reset_index(drop=True)
        # print(len(df))
        return df
    
    def split_data(self, df):
        train_df, test_valid_df = train_test_split(df, train_size=self.train_size, shuffle=True, random_state=123)
        test_df, valid_df = train_test_split(test_valid_df, train_size=self.test_size / (1 - self.train_size), shuffle=True, random_state=123)
        # print('train_df length:', len(train_df), 'test_df length:', len(test_df), 'valid_df length:', len(valid_df))
        return train_df,test_df,valid_df
    
    def create_data_generators(self, train_df, test_df, valid_df, img_size=(224, 224), batch_size=32, test_batch_size=50):
